...
class MyDataset(Dataset):
    ...
    input_typeに応じて処理を自由に分岐させてください．（今回はimgのみ．）
    """
    ...
    def _pca(self, X, input_type, n_components):
        ...
        if input_type == "features":
            if self.pca is None: # train
                pca.fit(X)
            X = pca.transform(X)
        # 複数チャンネルの画像でPCAしたければ以下のように．
        # たとえば，マルチスペクトル画像でチャンネルを削減したい場合など．
        elif input_type == "img":
            b, c, h, w = X.shape
