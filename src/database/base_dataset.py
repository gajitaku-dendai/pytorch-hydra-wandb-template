...
class BaseDataset(Dataset):
    ...
    def __init__(self, cfg: MyConfig, mode: str = "train",
        ...
        # データが空の場合のハンドリング
        if self.X.size == 0:
            print(f"Warning: {mode} dataset is empty after loading. Skipping preprocessing.")
            self.length = 0
            self.input_size = 0
            self.num_channels = 0
            # 空のTensorを設定して、後の処理でエラーにならないようにする
            self.X = torch.tensor([], dtype=torch.float32)
            self.y = torch.tensor([], dtype=torch.int64) # またはfloat32
            return

        self.sample_num = self.X.shape[0]
        if self.input_type == "img": # (B, C, H, W)
            self.original_size = (self.X.shape[2], self.X.shape[3])
        elif self.input_type == "features": # (B, F)
        ...
        if cfg.data.standardize_type != "none":
            ...
            # stdsが0の要素がある場合、除算エラーを防ぐ
            stds_safe = np.where(self.stds == 0, 1e-8, self.stds) # 0の代わりに小さい値を使う
            self.X = (self.X - self.means) / stds_safe
        ...
        if self.input_type == "features":
            # PCAで特徴量数が変わる可能性があるので、形状から取得
            self.input_size = self.X.shape[1]
            self.num_channels = 1
        elif self.input_type == "img":
            # PCAでチャンネル数が変わる可能性があるので、形状から取得
    ...
    def _load_data(self, mode, input_type):
        ...
        if self.cfg.data.use_kfold:
            for i in range(self.cfg.data.kfold_n_splits):
                if (mode == "test" and i != self.now_fold) or \
                   (mode != "test" and i == self.now_fold):
                    continue
            ...
            # ファイルが存在するか確認
            X_file_path = f"{base_path}/{mode}_X.npy"
            y_file_path = f"{base_path}/{mode}_y.npy"
            if not os.path.exists(X_file_path):
